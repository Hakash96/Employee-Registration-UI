...
export class EmpListComponent implements OnInit {
  ...
  onSearch() {
    this.employerList = this._employerList.filter((employer: Employer) => {
      const checkName = employer.employeeName.toLowerCase().includes(this.searchText.toLowerCase());
      const checkPhone = employer.mobileNum.toLowerCase().includes(this.searchText.toLowerCase());
